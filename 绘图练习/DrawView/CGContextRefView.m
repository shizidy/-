...
#pragma mark ========== 画虚线 ==========
    ...
    CGContextMoveToPoint(ctx, 0, 490);
    CGContextAddLineToPoint(ctx, self.frame.size.width - 3, 490);
    CGContextStrokePath(ctx);
    
    CGContextSetLineWidth(ctx, 2.0);
    CGFloat lengths1[] = {10,10,5};//第一个数为实线长，第二个为空隙长度，第三个实线长度，第四个空隙长度，三个数一直循环
    CGContextSetLineDash(ctx, 0, lengths1, 3);
    CGContextMoveToPoint(ctx, 0, 530);
    CGContextAddLineToPoint(ctx, self.frame.size.width,530);
    CGContextStrokePath(ctx);
    
    
    /*图片*/
    UIImage *image = [UIImage imageNamed:@"icon_6.jpeg"];
    [image drawInRect:CGRectMake(60, 340 + 100, 60, 60)];//在坐标中画出图片
    //    [image drawAtPoint:CGPointMake(100, 340)];//保持图片大小在point点开始画图片，可以把注释去掉看看
    CGContextDrawImage(ctx, CGRectMake(150, 340 + 100, 60, 60), image.CGImage);//使用这个使图片上下颠倒了，参考http://blog.csdn.net/koupoo/article/details/8670024
